...
rrf_rates_times <- function(tree.name = "", type=c("NEWICK", "NEXUS"), outgroup = "", filename = "", plot = TRUE){
  ...
  if (!library("ape",logical.return = TRUE)){
    stop("'ape' package not found, please install it to run rrf.rates.times.")
  }
  if (!library("phangorn",logical.return = TRUE)){
    stop("'phangorn' package not found, please install it to run rrf.rates.times.")
  }
  ...
  out.mat <- matrix(NA, ncol=6, nrow = t$Nnode+tips.num)
  colnames(out.mat) <- c("NodeLabel", "NodeId", "Des1", "Des2", "Rate", "Time")
  ...
  write.csv(data.frame(out.mat), file = paste0(filename, "_RRF_table.csv"), row.names = FALSE)
  ...
  # out.tree$node.label <- tips.num + c(1:out.tree$Nnode)
  out.tree$tip.label <- paste0(t$tip.label, "[&rate=", out.mat[1:tips.num, "Rate"], "]")
  out.tree$node.label <- paste0("[&rate=", out.mat[tips.num + c(1:out.tree$Nnode), "Rate"], "]")

  ape::write.nexus(out.tree, file = paste0(filename, "_RRF_timetree.nexus"), translate = FALSE)
  nexus.text <- readLines(paste0(filename, "_RRF_timetree.nexus"))
  nexus.text[grep("BEGIN TAXA", nexus.text):grep("END", nexus.text)[1]] <- gsub("\\[.*?\\]", "", nexus.text[grep("BEGIN TAXA", nexus.text):grep("END", nexus.text)[1]])
  write(nexus.text, file = paste0(filename, "_RRF_timetree.nexus"))

  ####### plot timetree with colored rates ########
  if (plot == TRUE){
    out.tree <- ape::read.nexus(paste0(filename, "_RRF_timetree.nexus"))
    rel.rates <- as.numeric(out.mat[,"Rate"])
    rate.bin <- seq(min(rel.rates), max(rel.rates), (max(rel.rates)-min(rel.rates))/10)
    mycol <- RColorBrewer::brewer.pal(10, "RdYlBu")
    rate.colr <- vector("character", length(rel.rates))
    for (i in 2:11){
      rate.colr[which(rel.rates >= rate.bin[i-1] & rel.rates <= rate.bin[i])] = mycol[i-1]
    }
    plot(out.tree, edge.color = rate.colr[match(out.tree$edge[,2], out.mat[,"NodeId"])])
  }
