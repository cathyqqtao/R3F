...
rrf_times <- function(tree.name = "", type=c("NEWICK", "NEXUS"), outgroup = "", filename = ""){
  ...
  if (zero.brlen >= 0.1){
    print(paste0("Warning: there are too many zero-length branches (>", floor(zero.brlen*100), "%) in the tree, node times calculation may not be reliable for nodes connecting to those branches."))
  }
